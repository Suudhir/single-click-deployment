...
def delta(costs):
    if (len(costs) > 1 and sum(costs[-7:]) >= 1 and sum(costs[-14:-7]) >= 1):
        # This only handles positive numbers
        result = ((sum(costs[-7:])/sum(costs[-14:-7]))-1)*100.0
    else:
        result = 0
    return result
...
def lambda_handler(event, context):
    ...
    report_type = event['time']
    if(report_type == 'Daily'):
        summary, buffer, data = daily_report_cost(group_by=group_by, length=length, cost_aggregation=cost_aggregation)
...
def daily_report_cost(group_by: str = "SERVICE", length: int = 5, cost_aggregation: str = "UnblendedCost", result: dict = None, yesterday: str = None, new_method=True):
    ...
    buffer = f"{'Service':{longest_name_len}} ${'Yday':8} {'∆%':>5} {'Last '}{n_days}{'d':31}\n"

    for service_name, costs in most_expensive_yesterday[:length]:
        buffer += f"{service_name:{longest_name_len}} ${costs[-1]:8,.2f} {daily_delta(costs):4.0f}% {sparkline(costs):31}\n"
    ...
    buffer += f"{'Total':{longest_name_len}} ${total_costs[-1]:8,.2f} {daily_delta(total_costs):4.0f}% {sparkline(total_costs):31}\n"
    ...
    if credits_expire_date:
        ...
        # summary = f"<@U01B1B3PNCW>, Cost for account {account_name} was ${total_costs[-1]:,.2f}"
        summary = f"<@U01B1B3PNCW>, Yesterday's cost for account {account_name} was ${total_costs[-1]:,.2f}"
...
def report_cost(group_by: str = "SERVICE", length: int = 5, cost_aggregation: str = "UnblendedCost", result: dict = None, yesterday: str = None, new_method=True):
    ...
    # Sort the map by yesterday's cost
    most_expensive_last_week = sorted(cost_per_day_by_service.items(), key=lambda i: sum(i[1][-7:]), reverse=True)
    
    
    service_names = [k for k,_ in most_expensive_last_week[:length]]
    longest_name_len = len(max(service_names, key = len))

    buffer = f"{'Service':{longest_name_len}} ${'LWeek':8} {'∆%':>5} {'Last '}{n_days}{'d':31}\n"

    for service_name, costs in most_expensive_last_week[:length]:
        buffer += f"{service_name:{longest_name_len}} ${sum(costs[-7:]):8,.2f} {delta(costs):4.0f}% {sparkline(costs):31}\n"

    total_costs = [0.0] * n_days
    for day_number in range(n_days):
        for service_name, costs in most_expensive_last_week:
            try:
                total_costs[day_number] += costs[day_number]
            except IndexError:
                total_costs[day_number] += 0.0

    buffer += f"{'Total':{longest_name_len}} ${sum(total_costs[-7:]):8,.2f} {delta(total_costs):4.0f}% {sparkline(total_costs):31}\n"

    cost_per_day_by_service["total"] = sum(total_costs[-7:])
    ...
    if credits_expire_date:
        ...
        relative_to_budget = (sum(total_costs[-7:]) / allowed_credits_per_day) * 100.0
        ...
        summary = (f"{emoji} Yesterday's cost for {account_name} ${sum(total_costs[-1]):,.2f} "
                   f"is {relative_to_budget:.2f}% of credit budget "
                   f"${allowed_credits_per_day:,.2f} for the day."
                  )
    else:
        # summary = f"Yesterday's cost for account {account_name} was ${total_costs[-1]:,.2f}"
        summary = f"<@U01B1B3PNCW>, Cost for account {account_name} was ${sum(total_costs[-7:]):,.2f}"
